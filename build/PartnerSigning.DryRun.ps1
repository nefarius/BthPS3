...
$root = Join-Path ([IO.Path]::GetTempPath()) ('bthps3-partner-dryrun-' + [guid]::NewGuid().ToString('N'))
...
function Write-SubmissionJson([switch] $AsArray) {
    ...
    $submission = [ordered]@{
        ...
        name           = 'BthPS3 dry run submission'
        type           = 'initial'
...
switch ("$noun $verb") {
    ...
    'submission download' {
        ...
        Set-Content -LiteralPath (Join-Path $stage 'BthPS3.sys') -Value 'signed-package'
        Set-Content -LiteralPath (Join-Path $stage 'BthPS3PSM.sys') -Value 'signed-filter'
...
function New-Sandbox([string] $Name) {
    ...
    @{ tag = 'v2.12.0'; setupVersion = '2.12.0'; driverVersion = '2.12.0.2012' } |
    ...
    Set-Content -LiteralPath (Join-Path $dir 'submission/BthPS3_2.12.0.2012.cab') -Value 'ev-signed-cab'
...
try {
    ...
    Assert-Equal $result.tag 'v2.12.0' 'fresh: result records the tag'
    ...
    Assert-True ($unwrappedNames -contains 'BthPS3.sys') 'fresh: signed zip contains profile driver files'
    Assert-True ($unwrappedNames -contains 'BthPS3PSM.sys') 'fresh: signed zip contains filter driver files'
