...
<#
.SYNOPSIS
    Resolves the BthPS3 driver/setup versions for a GitHub Actions run.

.DESCRIPTION
    Release tags must be exactly vMAJOR.MINOR.PATCH (example: v2.12.0). Those
...
function ConvertTo-BthPS3ReleaseVersion {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [AllowEmptyString()]
    ...
    if ($isTagRef) {
        if ($tag -notmatch '^v(\d+)\.(\d+)\.(\d+)$') {
            throw "Release tags must be vMAJOR.MINOR.PATCH (example: v2.12.0). Got: '$tag'"
        }
...
function Write-BthPS3ReleaseVersionOutputs {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        $Version,
